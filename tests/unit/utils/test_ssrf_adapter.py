...
def test_mount_ssrf_pinning_trusts_passed_urls_for_proxy_routing(monkeypatch):
    """mount_ssrf_pinning's trusted_urls affect proxy routing (send()), not the
    non-global rejection itself, which reads JIRA_URL/CONFLUENCE_URL/
    MCP_ALLOWED_URL_DOMAINS directly at connect time."""
    for k in ("JIRA_URL", "CONFLUENCE_URL", "MCP_ALLOWED_URL_DOMAINS"):
        monkeypatch.delenv(k, raising=False)
    session = requests.Session()
    mount_ssrf_pinning(session, "https://on-prem.internal")

    adapter = session.get_adapter("https://on-prem.internal")
    assert isinstance(adapter, SsrfPinningAdapter)
    assert "on-prem.internal" in adapter._trusted_hosts

    request = requests.Request("GET", "https://on-prem.internal").prepare()
    proxies = {"https": "http://proxy.example.test:8080"}
    response = requests.Response()
    with patch.object(HTTPAdapter, "send", return_value=response) as send:
        assert adapter.send(request, proxies=proxies) is response
