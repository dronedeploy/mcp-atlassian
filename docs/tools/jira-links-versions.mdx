...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `issue_key` | `string` | Yes | The key of the issue that has the remote link (e.g., 'PROJ-123', 'ACV2-642') |
| `link_id` | `string` | Yes | The ID of the remote link to remove (from create response or issue remotelinks) |
